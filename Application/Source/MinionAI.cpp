...
void MinionAI::update(double dt)
{
	...
	if (!CollisionMesh_->isCollide(Player::getInstance()->CollisionMesh_)) //if bots havent collide with player
	{
		...
		for (size_t i = 0; i < (sizeof WorldScene::goatMinionPool) / sizeof(*WorldScene::goatMinionPool); i++)
		{
			if (WorldScene::goatMinionPool[i] != this && 
				WorldScene::goatMinionPool[i]->active && 
				projected.isCollide(WorldScene::goatMinionPool[i]->CollisionMesh_))
				return;
		}
